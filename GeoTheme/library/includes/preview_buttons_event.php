...
{
	$form_action_url = home_url().'/?ptype=paynow_event';
}else
{
	$form_action_url = get_ssl_normal_url(home_url().'/?ptype=paynow_event');
}
?>
...
jQuery('input:submit').click(function(){
	...
	jQuery('input:submit').after('<input type="submit" name="Submit and Pay" value="<?php echo SUBMITTING;?>" class="fr b_cancel" disabled="true" />');
});
</script>

<a href="<?php echo home_url();?>/?ptype=post_event&backandedit=1<?php if($_REQUEST['pid']){ echo '&pid='.$_REQUEST['pid'];} if($_SESSION['property_info']['price_select']){echo '&pkg='.$_SESSION['property_info']['price_select'];} ?><?php if($_REQUEST['renew']){echo '&renew=1';}?><?php if($_REQUEST['upgrade']){echo '&upgrade=1';}?><?php if($_POST['a_businesses']){echo '&linkid='.$_POST['a_businesses'];}?>" class="b_goback fl" ><?php _e(PRO_BACK_AND_EDIT_TEXT);?></a>
