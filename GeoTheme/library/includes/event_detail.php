...
<?php }?>
	 ...
	 <?php if(get_post_meta($post->ID,'video',true) || $video){?>
      ...
      <div class="register_info" <?php if((get_post_meta($post->ID,'reg_desc',true)!='' && $pkg_limit['reg_desc_pkg']==1) || ($reg_desc!='' && $pkg_limit['reg_desc_pkg']==1)){}else{ echo "style='display:none'";}?>>     

           

      	<?php if(($reg_desc || get_post_meta($post->ID,'reg_desc',true)) && $pkg_limit['reg_desc_pkg']==1){?>

      		<?php if($preview){echo $reg_desc;}else{echo get_post_meta($post->ID,'reg_desc',true);}?> 

      <?php }?> 

      <?php if(($reg_fees || get_post_meta($post->ID,'reg_fees',true)) && $pkg_limit['reg_fees_pkg']==1){?>
...
<?php if($pkg_limit['link_business_pkg']==1 && get_event_place() && !$preview){ ?>
