...
function search_filter($local_wp_query) {
	...
	{		
	add_filter('posts_fields_request', 'featured_q');
...
MATCH ($wpdb->posts.post_title, $wpdb->posts.post_content) AGAINST ('$s') AS score ";
	...
	function featured_q($fields){
	global $wpdb;
	$s = mysql_real_escape_string($_REQUEST['s']);
	return $fields." , (select $wpdb->postmeta.meta_value from $wpdb->postmeta where $wpdb->postmeta.post_id=$wpdb->posts.ID and $wpdb->postmeta.meta_key like \"is_featured\")+0 AS featured";
	}
...
$get_sort = mysql_real_escape_string($_REQUEST['sort']);
		...
		if($_REQUEST['etype']=='')
			...
			elseif($_REQUEST['etype']=='upcoming')
			{
				...
				$orderby = "  featured DESC, $wpdb->posts.post_date DESC";}else{
...
function searching_filter_orderby($orderby) {
	...
	$orderby .= "  (titlematch * 1.5 + is_featured * 5 + exacttitle * 10 + score) DESC, featured desc,  $wpdb->posts.post_title ";
	return $orderby;	
}
