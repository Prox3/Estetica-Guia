...
<?php if ( is_home() ) { ?><?php  bloginfo('description'); ?>&nbsp;|&nbsp;<?php bloginfo('name'); if($_SESSION['location_name']){echo ' - '.$_SESSION['location_name'];} ?><?php } ?>
...
if($ct_on && file_exists(get_stylesheet_directory().'/library/js/post.custom.js')){?>
<script type='text/javascript' src='<?php echo get_bloginfo('stylesheet_directory'); ?>/library/js/post.custom.js'></script>
<?php }else {?>
...
<?php /*?><?php if(wp_nav_menu( array( 'theme_location' => 'top_menu','fallback_cb' =>false,'echo' =>false))){wp_nav_menu( array( 'theme_location' => 'top_menu','fallback_cb' =>false,'echo' =>true));}
...
<li class="userin"><a href="<?php echo get_ssl_normal_url(home_url()."/?ptype=login&amp;page1=sign_in");?>" class="signin"><?php echo SIGN_IN_TEXT;?></a></li>
<?php }?>
</ul>
<?php 
...
<div id="shadow_navigator"></div>
</div> <!-- header #end -->

<?php  if(wp_nav_menu( array( 'theme_location' => 'main_menu','fallback_cb' =>false,'echo' =>false))){
	echo '<div id="categories_strip"><div id="rmenu">'.__('Menu').'</div>';
...
<div id="fix_loc_select"><?php dynamic_sidebar('Header Navigation Right');}else{ echo '</div><div id="categories_strip"><div id="rmenu">'.__('Menu').'</div><div id="main_nav_menu">'; ?>
<script type='text/javascript'>jQuery('#categories_strip2').hide();</script>
<ul>
<!--<li class="<?php if ( is_home() ) { ?>current_page_item<?php } ?> home" ><a href="<?php echo get_option('home'); ?>/"><?php _e('Home');?></a></li>-->
<li class="cat-item" ><a href="<?php echo get_option('home'); ?>/" style="padding:8px 15px 0px 4px !important;">PÁGINA INICIAL</a></li>
...
?> 
<label class="lb_region">Regiões:</label>
<?php if(get_option('is_user_addevent')=='0'){}else{ ?>
<li class="<?php if ( is_home() && (isset($_REQUEST['ptype']) && $_REQUEST['ptype']=='post_listing') ) { ?>current_page_item<?php } ?>" ><a href="<?php echo home_url(); ?>/?ptype=post_listing"><?php echo ADD_LISTING;?></a></li>
<?php }?>
<?php if(get_option('is_user_eventlist')=='0'){}else{ ?>
<li class="<?php if ( is_home() && (isset($_REQUEST['ptype']) && $_REQUEST['ptype']=='post_event')) { ?>current_page_item<?php } ?>" ><a href="<?php echo home_url(); ?>/?ptype=post_event"><?php echo ADD_EVENT;?></a></li>
<?php }?>
</ul>
...
<div id="location_select_wrapper_header"></div>
<div id="location_select" >
<?php if(get_option('ptthemes_enable_multicity_flag')){ get_location_menu();} ?>
</div>
<div id="location_select_wrapper_footer"></div>
</div>
...
<script>
    ...
    $("#content").find(".widget").eq(1).find("h3:eq(0)").css({
			"background": "url(http://www.esteticaguia.provisorio.ws/wp-content/themes/GeoTheme/images/bg_rating_best.png) 0 10px no-repeat",
			"padding-left": "28px",
			"color": "#c748c3"
		});
		$(".wide_comment_text").find("a:eq(0), a:eq(1), a:eq(2)").css({
			"font-family": "'Titillium Web', sans-serif",
			"font-size": "20px",
			"color": "#333333",
			"font-weight": "500"
		});
		$(".wide_comment_text").find("a:eq(1)").css({
			"color": "#4b99d1"
		});
		$(".wide_comment_text").find("a:eq(2)").css({
			"font-size": "12px",
			"font-weight": "400"
		});
		$("#sidebar").find(".widget:eq(1)").find("h3").css({
			"color": "#30babc"
		});
		$("#sidebar").find(".widget:eq(1)").addClass("w_estetica");
		$(".w_estetica ul").find("li").find("a:eq(1)").css({
			"font-family": "'Titillium Web', sans-serif",
			"font-size": "14px",
			"color": "#666666",
			"font-weight": "600",
		...
		$(".subscribe_form").find("input").eq(0).css({
			"border": "none",
			"background": "transparent url(http://www.esteticaguia.provisorio.ws/wp-content/themes/GeoTheme/images/bg_newsletter.png) no-repeat",
			"height": "30px",
			"width": "170px",
			"padding": "0 0 0 10px"
		});
		$(".subscribe_form").find(".btn_submit").css({
			"display": "none"
		});
		$(".c_list .category_list_view li .content .address").next().addClass("divAddress");
		$(".c_list .category_list_view li .content").find("p:eq(2)").addClass("descListPlace");
		...
		$(".company_info, .company_info2").hide();
		...
		$(".singlePlace #content .single_post_advt").hide();
		$(".singlePlace #content .realated_post").hide();
  });
</script>