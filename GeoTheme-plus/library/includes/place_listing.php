...
if($cat_description){echo str_replace('%location_name%', $_SESSION['location_name'], $cat_description);}?>
        ...
        <div class="clearfix"></div>
             	<div class="city-neighborhood-selector">
		    <span class="neighborhood-selector">
		    	<form id="neighborhood-selector-form" action="http://<?php echo $_SERVER['HTTP_HOST'].$_SERVER['SCRIPT_NAME']; ?>" method="get">
			    <label><?php echo NEIGHBOURHOOD;?> </label>
<?php  
	$mr_request=$_REQUEST;
	if(!$city_id){$city_id = mysql_real_escape_string ($_SESSION['multi_city']);}
/*	$neighborhood_select=get_multihood_select_dl('area','post_hood_id',$city_id,' class="textfield textfield_x" ','','',$post_hood_id); 
 *	$neighborhood_select=preg_replace(array('/(<select.*?)>/','/<option.*?>/','/<option>'.$mr_request['area'].'<\/option>/'),array('$1 onchange="this.form.submit()" >','<option>','<option selected="selected">'.$mr_request['area'].'</option>'),$neighborhood_select);
 */
	$neighborhood_select=get_multihood_select_dl('area','post_hood_id','',' class="textfield textfield_x" onchange="this.form.submit()"','',get_area_selector_options($mr_request['area'],'',$att='', $city_id,'hoodname'),$post_hood_id); 
	echo $neighborhood_select;
	foreach ( array('area','multi_city') as $mr_remove_key ) {
	    if (isset($mr_request[$mr_remove_key])) {
		unset($mr_request[$mr_remove_key]);
	    }
	}
	foreach ($mr_request as $mr_request_key => $mr_request_value) {
	    echo '<input type="hidden" name="'.$mr_request_key.'" value="'.$mr_request_value.'" />'.PHP_EOL;
	}
?>
		    <input id="neighborhood-selector-submit" type="submit" value="Go!" /><script type="text/javascript">

jQuery(function($){$('#neighborhood-selector-submit').hide(); })</script>
		        </form>
		    </span><!-- class="neighborhood-selector" --> 
		</div><!-- class="city-neighborhood-selector" -->
		<div class="sort_by sort_by_dd"   <?php if(get_option('ptthemes_cat_sort_dd')){}else{ echo 'style="display:none;"';}?>><?php _e('Sort by');?> :
...
$default_img = get_post_default_img($post->ID,  'place');
...
<a class="post_img" href="<?php the_permalink(); ?>"><img src="<?php echo $default_img;?>" alt="<?php the_title(); ?>" title="<?php the_title(); ?>"  /></a><?php }?> 
                ...
                <p class="grid-excerpt"><?php echo excerpt(15); ?> </p>
                <p class="timing">
                <?php
